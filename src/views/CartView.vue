<template>
  <div class="cart-view w-full max-w-md mx-auto bg-dbd-off-white min-h-screen flex flex-col font-montserrat">
    ...
    <div class="content-container bg-dbd-off-white flex-1 flex flex-col relative">
      ...
      <!-- Bottom Section: Total and Action Buttons (part of layout, not fixed) -->
      <div v-if="cartItemsCount > 0" class="px-3.5 pt-4 pb-6 bg-dbd-off-white border-t border-gray-100">
        <!-- Total Section -->
        <div class="flex items-center justify-center gap-1 mb-4">
          <span class="text-dbd-dark font-semibold text-xl leading-7">Total to pay:</span>
          <span class="text-dbd-primary font-semibold text-xl leading-7">${{ cartTotal.toLocaleString() }}</span>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            @click="$router.go(-1)"
            class="flex items-center justify-center gap-2 h-12 px-5 bg-dbd-off-white border border-dbd-gray rounded-full flex-shrink-0 min-w-24"
          >
          ...
          <button
            ...
            class="flex items-center justify-center h-12 px-8 bg-gradient-to-r from-dbd-primary to-blue-500 rounded-full text-white font-bold text-lg flex-1"
          >
            Buy Forevers
          </button>
        </div>
