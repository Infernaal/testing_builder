<template>
  <div class="profile-view w-full h-screen font-montserrat relative overflow-hidden">
    
    <!-- Profile Avatar Button (как в дизайне Figma) -->
    <div class="absolute left-4 top-6 z-20">
      ...
      >
        <!-- Background Circle -->
        <div class="w-12 h-12 rounded-full border border-[#2019CE] bg-[#F4F3FF] flex items-center justify-center relative">
          <!-- Close Icon (X) или Menu Icon -->
          <div v-if="isProfileMenuOpen" class="text-[#4B4D50]">
          ...
          <div v-else class="relative">
            <!-- Avatar -->
            <div class="w-8 h-8 rounded-full border border-[#7E73D6] overflow-hidden">
              <img 
          ...
          <!-- Arrow Indicator -->
          <div class="absolute -right-1 top-5 w-3 h-3 bg-[#FAFAFA] border border-[#E8E8E8] rounded-full flex items-center justify-center">
            <svg 
              width="8" 
              height="8" 
              viewBox="0 0 12 12" 
        ...
        <!-- Profile Text Below -->
        <div class="text-[#4B4D50] text-xs font-medium text-center mt-1 w-10">Profile</div>
      </button>
    </div>

    <!-- Background Overlay когда меню открыто -->
    <Transition
      name="overlay"
    ...
    >
      <div
        ...
        class="fixed inset-0 bg-[#595959]/10 backdrop-blur-[12px] z-30"
        @click="closeProfileMenu"
      ></div>
    </Transition>

    <!-- Profile Dropdown Menu (точно как в Figma) -->
    <Transition
      name="slide-up"
    ...
    >
      <div
        v-if="isProfileMenuOpen"
        class="fixed inset-x-0 bottom-0 z-40 max-w-md mx-auto px-2"
        style="height: calc(100vh - 40px);"
      >
        <div 
          class="w-full h-full rounded-t-[20px] border border-white/8 relative overflow-hidden"
          style="background: linear-gradient(94deg, #120B81 33.64%, #09074E 76.52%); backdrop-filter: blur(32px);"
        >
          
          <!-- Profile Header Section (точно как в Figma) -->
          <div class="w-full h-24 bg-[#605F87]/24 border border-[#D8D8D8] backdrop-blur-sm rounded-[50px_20px_20px_50px] mt-2 mx-2 p-2 relative" style="width: calc(100% - 16px);">
          ...
          <!-- Menu Items Section -->
          <div class="px-3 space-y-1 flex-1 overflow-y-auto mt-4" style="height: calc(100% - 280px);">
            <!-- Calculator -->
            ...
            <div class="absolute right-2 top-1/2 w-[2px] h-6 bg-[#E2E2E2] border border-[#B7B7B7] rounded-[30px] opacity-40"></div>
          </div>

          <!-- Bottom Section -->
          <div class="absolute bottom-0 left-0 right-0 p-3 space-y-3">
            <!-- Start Block - Upgrade Section (точно как в Figma) -->
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <!-- Language Selector с флагом (точно как в Figma) -->
              <button 
...
const handleClickOutside = (event) => {
  // Logic for closing menu when clicking outside
  if (!event.target.closest('.profile-view')) {
    closeProfileMenu()
  }
}
...
.profile-view {
  ...
  /* Full screen height для Telegram WebApp */
  height: 100vh;
  height: 100dvh; /* Dynamic viewport height для mobile */
  overflow: hidden;
...
/* Hover effects для interactive elements */
button:hover {
...
/* Safe area support для devices with notches */
