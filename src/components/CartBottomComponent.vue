<template>
  <div class="cart-bottom-component bg-gray-100 px-3.5 pt-4 pb-2">
    ...
    <div class="action-buttons flex items-center gap-2.5">
      ...
      <button
        @click="$emit('back')"
        class="back-button flex items-center justify-center gap-1.5 h-10 px-4 bg-dbd-off-white border border-dbd-gray rounded-full flex-shrink-0 transition-all duration-200 hover:bg-gray-100 min-w-[90px]"
      >
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" class="text-dbd-gray">
        ...
        <span class="text-dbd-gray font-medium text-xs leading-4">Back</span>
      </button>

      <!-- Buy Forevers Button -->
      <button
        @click="$emit('purchase')"
        class="buy-button flex items-center justify-center h-10 px-6 rounded-full text-white font-semibold text-base leading-5 flex-1 transition-all duration-200"
